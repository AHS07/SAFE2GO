"""Repeated seatbelt violation detector.

Fires when >= 2 seatbelt incidents occur within a shift.
Incident-driven, not tick-driven.
"""
...
class RepeatedSeatbeltDetector:
    """Counts seatbelt incidents within a shift."""
    ...
    def record_incident(self, event_ts: datetime) -> DetectionEvent | None:
        ...
        threshold = get_thresholds().behavior.repeated_seatbelt_count
        self._count += 1
        ...
        if self._count >= threshold and not self._emitted:
            ...
            return DetectionEvent(
                event_type=BehaviorEventType.REPEATED_SEATBELT_VIOLATION.value,
